...
const Sidebar = ({
  ...
  return (
    <>
      {backdrop && (
        <div
          className={`backdrop bg-dark z-[100] fixed top-0 right-0 bottom-0 left-0 w-full ${
      ...
      >
        <header className={`sidebar-header`}>
          <CloseButton
            ...
            layout="circle"
            size="md"
            className={`fixed top-3 ${closePosition === "left" ? "left-3" : "right-3"}`}
          />
        </header>
        <div className={`sidebar-content max-h-full overflow-y-auto p-0 pt-8`}>{children}</div>
      </aside>
    </>
  );
};
